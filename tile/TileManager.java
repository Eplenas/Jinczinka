        ...
        for (int dx = -radius; dx <= radius; dx++) {
            for (int dy = -radius; dy <= radius; dy++) {
                ...
                if (worldX >= 0 && worldX < jz.maxWorldCol && worldY >= 0 && worldY < jz.maxWorldRow) {
                    ...
                    int dxFromCenter = screenX + tileSize / 2 - centerX;
                    int dyFromCenter = screenY + tileSize / 2 - centerY;
                    ...
                    // Calculate two layers of transparency
                    if (distanceFromCenter > radius * tileSize) {
                        // Outer edge of the circle: Lighter transparency (e.g., 50%)
                        transparency = 0.5f;
                    } else if (distanceFromCenter > (radius - 1) * tileSize) {
                        // Square layer around the circle: Slightly transparent (e.g., 75%)
                        transparency = 0.75f;
                    }
    
                    // Apply transparency to the entire rendering
